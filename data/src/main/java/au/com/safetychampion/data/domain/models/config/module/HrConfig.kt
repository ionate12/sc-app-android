...
class HrConfig(config: Configuration) : BaseConfig(config.valueMaps()) {
    init {
        if (config.type != ModuleType.HR) {
            throw IllegalArgumentException("set wrong params, module Type must be HR")
        }
    }
}
