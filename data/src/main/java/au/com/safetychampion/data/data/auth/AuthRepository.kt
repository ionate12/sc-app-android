...
class AuthRepository : BaseRepository(), IAuthRepository {
    ...
    override suspend fun login(loginPL: LoginPL): Result<LoginResponse> {
        return AuthApi.Login(loginPL)
            .call<JsonObject>(objName = "")
            .map { customGson.fromJson(it, LoginResponse::class.java) }
            .doOnSucceed {
                updateToken(it)
    ...
    override suspend fun mfaVerify(mfaVerifyPL: MfaVerifyPL, loginPL: LoginPL): Result<LoginResponse> {
        return AuthApi.MfaVerify(mfaVerifyPL)
            .call<JsonObject>(objName = "")
            .map { customGson.fromJson(it, LoginResponse::class.java) }
            .doOnSucceed {
                updateToken(it)
    ...
    private suspend fun storeUserCredential(loginPL: LoginPL) {
        ...
        appDataStore.store(StoreKey.UserCredential, setOf(loginPL.email, loginPL.password))
    }
